...
ringRings: Array.from({length:10}, (_,i)=>({
  ...
  function getExportPayload(){
    const out = deepClone(pickPlanetParams(params));

    // --- Scale-invariant export (simulator uses desiredR):
    // Keep a reference radius for tuning (editor look), but do NOT export physical size.
    const radiusRef = Number(params.radius || 6.0);
    out.radiusRef = radiusRef;
    // Remove legacy absolute radius from exports
    if ('radius' in out) delete out.radius;

    // Convert seaHugBand from world-units to relative (fraction of radius)
    if ('seaHugBand' in out) {
      const v = Number(out.seaHugBand);
      if (Number.isFinite(v) && radiusRef > 1e-9) {
        out.seaHugBandRel = v / radiusRef;
      }
      delete out.seaHugBand;
    }

    warnIfAtmosMissing(out);
  ...
  function applyImportedParams(imported){
    ...
    // New format: radiusRef (editor tuning radius). We do NOT keep physical radius in editor exports.
    if (typeof filtered.radiusRef === 'number' && Number.isFinite(filtered.radiusRef) && filtered.radiusRef > 0) {
      params.radius = filtered.radiusRef;
      delete filtered.radiusRef;
    } else if (typeof filtered.radius === 'number' && Number.isFinite(filtered.radius) && filtered.radius > 0) {
      // Legacy json: treat radius as radiusRef for backward compatibility
      params.radius = filtered.radius;
      delete filtered.radius;
    } else {
      // Keep current editor radius if not provided
    }

    // New format: seaHugBandRel (fraction of radius)
    if (typeof filtered.seaHugBandRel === 'number' && Number.isFinite(filtered.seaHugBandRel)) {
      params.seaHugBand = filtered.seaHugBandRel * Number(params.radius || 6.0);
      delete filtered.seaHugBandRel;
    }
