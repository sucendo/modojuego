  ...
  "Sagittarius A*": {
    ...
    orbits: "Sagittarius A*", kind: "black_hole", radius: 1000, emissive: new BABYLON.Color3(0.05, 0.0, 0.1)  
      ...
      planets: {
		...
		"Alp2Cen A-b": {
			// UBICACIÓN: Zona Habitable de Alpha Centauri A (Estrella G2V)
			// DISTANCIA: ~1.25 UA (Equivalente a la Tierra pero ajustado a la mayor luminosidad de A)
			periapsis: 175000000,
			apoapsis: 195000000,
			
			// TAMAÑO: Una Súper-Tierra rocosa (basado en lo que se sospecha de los candidatos C1)
			size: 8500,        // Diámetro mayor que la Tierra
			
			color: 0x7a8b99,   // Gris azulado (atmósfera con nubes y océanos)
			orbits: "Alpha Centauri A",
			
			// PERIODO: Al estar más lejos, su año es más largo
			orbitalPeriod: 4500, 
			...
			lastPerihelion: "2024-01-01",
			rotationAxis: { x: 0, y: 1, z: 0 },
			axialTilt: 23.0,    // Inclinación tipo terrestre para tener estaciones
			rotationPeriod: 1.1, // Día de unas 26-27 horas
			kind: "planet",
			radius: 1.4,       // Radio de 1.4 veces el terrestre (límite para ser rocoso)
			jsonFile: "alp2cen_a-b.json"
		},

		"Alp2Cen B-b": {
			// Planeta más interno, muy rápido y caliente
			periapsis: 25581236,
			apoapsis: 31265955,
			size: 7800,
			color: 0x5c4033,
		...
		"Alp2Cen B-c": {
			...
			kind: "planet",
			radius: 1.15,
			jsonFile: "alp2cen_b-c.json"
		},

		"Alp2Cen B-d": {
			// Actúa como frontera antes de la zona habitable
			periapsis: 71717219,
			apoapsis: 80872609,
			size: 5700,
			color: 0x7b3f00,
			...
			kind: "planet",
			radius: 0.95,
			jsonFile: "alp2cen_b-d.json"
		},

		"Ecaz": {
			// UBICACIÓN: Zona Habitable de Alpha Centauri B (Estrella tipo K)
			// DISTANCIA: Optimizada para permitir agua líquida y vegetación exuberante
			periapsis: 104720000, 
			apoapsis: 119680000,
			size: 6100,              // Casi idéntico a la Tierra
			color: 0x2e8b57,         // Verde bosque profundo (Lore: Madera de niebla)
			orbits: "Alpha Centauri B", 
			orbitalPeriod: 2200,      // Periodo más largo por ser el más externo
			inclination: 1.8, 
			argumentOfPeriapsis: 240.0, 
			longitudeOfAscendingNode: 15.0,
			lastPerihelion: "2024-08-15",
			rotationAxis: { x: 0, y: 1, z: 0 }, 
			axialTilt: 18.0,         // Estaciones suaves para un clima forestal estable
			rotationPeriod: 0.98,    // Día de ~23.5 horas
			kind: "planet",
			radius: 1.05,            // Gravedad ~1g
			jsonFile: "ecaz.json"
		},
        "Proxima d": {
			periapsis: 3877577,
      ...
      satellites: {
		...
		"Luna": {
		  ...
		  orbits: "Tierra", orbitalPeriod: 27.321661,
		  
		  // JPL satellite mean elements (J2000 / 2000-01-01.5 TDB)
		  inclination: 5.16,
		  argumentOfPeriapsis: 318.15,
		  longitudeOfAscendingNode: 125.08,
		  
		  epochJD: 2451545.0,
		  semiMajorAxisKmAtEpoch: 384400.0,
		  eccentricityAtEpoch: 0.0554,
		  inclinationAtEpoch: 5.16,
		  
		  // Derivados de los elementos medios JPL
		  meanLongitudeAtEpoch: 218.50,                  // = Ω + ω + M  (mod 360)
		  meanLongitudeRateDegPerCentury: 481260.5226557353,
		  longitudeOfPerihelionAtEpoch: 83.23,           // en la Luna sería más correcto "perigeo/periapsis"
		  longitudeOfPerihelionRateDegPerCentury: 6003.001500750375,
		  longitudeOfAscendingNodeAtEpoch: 125.08,
		  longitudeOfAscendingNodeRateDegPerCentury: -1935.4838709677417,
		  
		  // Si mantienes tu esquema actual:
