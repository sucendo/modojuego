...
if (ui.toggleLabels) {
  ...
  ui.toggleLabels.addEventListener("change", () => {
    ...
    updateLabelVisibility(true);
      ...
      const LOD = {
        updateMs: 120,
        // Cuando estás más cerca que esto => objetivo = hi
        nearMul: 70,
        // Cuando estás más lejos que esto => objetivo = low
        farMul: 200,
        // Caps
      ...
      function desiredSubdivByDistance(dist, radius, baseRawSubdiv) {
        ...
        const t = _smoothstep((dist - near) / (far - near));
      ...
      function ensureLODSubdiv(b, targetSubdiv) {
        ...
        if (b.def.kind === "planet") {
          relinkMoonsParent(b.def.name, mesh);
        }
      ...
      function updateDynamicLOD(camPos) {
        ...
        // Target: surface => surfaceBody; si no, el rocky JSON más cercano
        let target = null;
        ...
        // Si cambia el target, los demás se fuerzan a LOW
        if (_lodActiveName && target && _lodActiveName !== target.def.name) {
          const prev = bodies.get(_lodActiveName);
          if (prev) {
            // al salir de target, lo dejamos en LOW "barato"
            const hasSpecific = !!prev.def.jsonFile && planetParamsByName.has(prev.def.name);
            const base = hasSpecific ? planetParamsByName.get(prev.def.name) : defaultPlanetParams;
            const raw = baseSubdivFromParams(base);
            const low = _clamp(LOD.lowCap, 2, Math.max(2, Math.min(raw, LOD.hiFloor)));
            ensureLODSubdiv(prev, low);
          }
          _lodActiveName = null;
        }

        if (target) {
          const hasSpecific = !!target.def.jsonFile && planetParamsByName.has(target.def.name);
          const base = hasSpecific ? planetParamsByName.get(target.def.name) : defaultPlanetParams;
          const raw = baseSubdivFromParams(base);
          const desired = (mode.value === "surface")
            ? _clamp(raw, LOD.hiFloor, LOD.hiCap)
            : desiredSubdivByDistance(targetDist, target.def.radius, raw);

          ensureLODSubdiv(target, desired);
