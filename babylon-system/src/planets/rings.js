...
export function makeRings(scene, planetMesh, opts) {
  ...
  // Keep rings in the same rendering group as the planet so depth behaves as expected.
  // Rings must NOT write depth, but MUST depth-test against the planet depth buffer.
  ...
  if (mat) {
    mat.disableDepthWrite = true;  // critical: don't overwrite depth buffer
