 ...
 * v4.2 · Combat Feel: evasión anticollision, energía, daño del F-22 y ritmo de combate.
...
const GAME = Object.freeze({
  ...
  playerManeuverDrag: 31,       // pérdida máxima de m/s² en maniobra muy fuerte
  playerEnergyFloorAirspeed: 148,
  playerWorldHeading: -Math.PI / 2, // rumbo de referencia: hacia la parte superior

  // Daño progresivo del F-22.
  playerIntegrity: 100,
  playerGunDamageMin: 18,
  playerGunDamageMax: 27,
  playerMissileDamageMin: 48,
  playerMissileDamageMax: 68,
  playerCollisionDamageMin: 76,
  playerCollisionDamageMax: 100,
  playerDamagedThreshold: 66,
  playerCriticalThreshold: 32,
  gravity: 9.80665,
  enemyMinSpeed: 170,
  enemyMaxSpeed: 345,
  ...
  // Prevención de colisiones: calcula el closest point of approach (CPA)
  // con la velocidad relativa y ordena una ruptura si el cruce previsto es peligroso.
  enemyAvoidLookAheadMin: 1.15,
  enemyAvoidLookAheadMax: 2.35,
  enemyAvoidPlayerBaseDistance: 62,
  enemyAvoidTrafficBaseDistance: 48,
  enemyAvoidMaxDetectionDistance: 560,

  // Director de intensidad: no elimina amenazas activas, solo evita encadenar
  // spawns y nuevos misiles cuando la carga de combate ya es muy alta.
  directorStressThreshold: 6.3,
  directorRecoveryMinSeconds: 5.5,
  directorRecoveryMaxSeconds: 8.5,
...
class PlaneScene extends Phaser.Scene {
  ...
  create() {
    this.state = {
      ...
      playerIntegrity: GAME.playerIntegrity,
      playerDamageState: 'OK',
      playerDamageSmokeTimer: 0,
      energyPercent: 100,
  ...
  updateWeaponLock(dt) {
    ...
    if (!candidate) {
      ...
      if (this.state.missionStarted) {
        this.showContextTip('track', 'KEEP TARGET AHEAD  //  TRACK → LOCK');
      }
    }
  ...
  showContextTip(key, text, duration = 2600) {
    if (!this.contextTips || this.contextTips[key] || this.state.gameOver) return;
    this.contextTips[key] = true;

    this.contextTipText?.destroy();
    this.contextTipText = null;

    const compact =
      this.hudPresentationMode === 'compact' ||
      this.hudPresentationMode === 'micro';

    const y = compact
      ? Math.max(38, this.scale.height - 46)
      : Math.max(76, this.scale.height - 74);
    ...
    const tip = this.add.text(
      ...
      text,
      {
        ...
        stroke: '#001018',
        strokeThickness: 4,
        backgroundColor: 'rgba(2,15,24,.68)',
        padding: { x: 8, y: 4 },
        align: 'center'
      }
    ).setOrigin(0.5, 1).setDepth(144).setScrollFactor(0);

    this.contextTipText = tip;
    ...
    this.tweens.add({
      ...
      duration: 420,
      onComplete: () => {
        if (this.contextTipText === tip) this.contextTipText = null;
        tip.destroy();
      }
    });
  }
  ...
  updateMissileWarning(dt) {
    ...
    if (this.state.missionStarted) {
      this.showContextTip(
        'flare',
        this.isMobileLayout
          ? 'MISSILE!  TURN HARD + TAP FLR'
          : 'MISSILE!  TURN HARD + C FOR FLARES'
      );
    }
        ...
        this.time.delayedCall(650, () => {
          if (!this.state.gameOver) {
            this.showContextTip(
              'move',
              this.isMobileLayout
                ? 'MOVE WITH JOYSTICK  //  SPD WITH + / −'
                : 'MOVE WITH ARROWS  //  Q/E SET SPEED'
            );
          }
        });
      }
  ...
  update(time, delta) {
    ...
    this.updatePlayerDamageState(dt);
    this.updateCombatDirector(dt);
  ...
  getPlayerDamageFactor() {
    if (this.state.playerDamageState === 'CRITICAL') return 0.78;
    if (this.state.playerDamageState === 'DAMAGED') return 0.90;
    return 1;
  }

  updatePlayerDamageState(dt) {
    if (!this.player?.active || this.state.gameOver) return;

    const critical = this.state.playerDamageState === 'CRITICAL';
    const damaged = this.state.playerDamageState === 'DAMAGED';

    if (!critical && !damaged) return;

    this.state.playerDamageSmokeTimer -= dt;
    if (this.state.playerDamageSmokeTimer <= 0) {
      this.spawnEnemyDamagePuff(this.player, critical);
      this.state.playerDamageSmokeTimer = Phaser.Math.FloatBetween(
        critical ? 0.055 : 0.16,
        critical ? 0.11 : 0.27
      );
    }
  ...
  updatePlayerAirspeed(dt) {
    ...
    const damageFactor = this.getPlayerDamageFactor();
    const maxChange = GAME.playerAcceleration * damageFactor * dt;

    if (before < this.state.targetAirspeed) {
      this.state.playerAirspeed = Math.min(
        this.state.targetAirspeed,
        before + maxChange
      );
    } else if (before > this.state.targetAirspeed) {
      this.state.playerAirspeed = Math.max(
        this.state.targetAirspeed,
        before - maxChange
      );
    }

    // Energía arcade: maniobrar fuerte cuesta velocidad. Al enderezar el avión,
    // la aceleración normal recupera progresivamente la velocidad seleccionada.
    const pvx = this.player?.body?.velocity.x || 0;
    const pvy = this.player?.body?.velocity.y || 0;
    const maneuverLoad = Phaser.Math.Clamp(
      Math.hypot(pvx, pvy) / Math.max(1, GAME.playerSpeed),
      0,
      1
    );

    const dragLoad = Math.pow(
      Phaser.Math.Clamp((maneuverLoad - 0.18) / 0.82, 0, 1),
      1.35
    );

    this.state.playerAirspeed = Math.max(
      GAME.playerEnergyFloorAirspeed,
      this.state.playerAirspeed -
        GAME.playerManeuverDrag * dragLoad * damageFactor * dt
    );

    const energyDenominator = Math.max(
      1,
      this.state.targetAirspeed - GAME.playerEnergyFloorAirspeed
    );
    this.state.energyPercent = Math.round(
      Phaser.Math.Clamp(
        (this.state.playerAirspeed - GAME.playerEnergyFloorAirspeed) /
          energyDenominator,
        0,
        1
      ) * 100
    ...
    if (
      this.state.energyPercent < 72 &&
      this.state.missionStarted
    ) {
      this.showContextTip(
        'energy',
        'HARD TURNS COST SPEED  //  STRAIGHTEN TO RECOVER'
      );
    }
    ...
    if (
      targetChanged ||
      Math.floor(before * 3.6) !== Math.floor(this.state.playerAirspeed * 3.6)
    ) {
      this.updateHud();
    }
  }
  ...
  updatePlayerMovement(dt) {
    const damageFactor = this.getPlayerDamageFactor();
    const controlFactor =
      this.state.playerDamageState === 'CRITICAL' ? 0.84 :
      this.state.playerDamageState === 'DAMAGED' ? 0.94 :
      1;
    ...
    if (inputX || inputY) {
      this.pointerTarget = null;
      const vector = new Phaser.Math.Vector2(inputX, inputY).normalize().scale(GAME.playerSpeed * controlFactor);
      desiredX = vector.x;
      desiredY = vector.y;
      hasCommand = true;
    } else if (
      ...
      desiredX = this.mobileMoveVector.x * GAME.playerSpeed * controlFactor;
      desiredY = this.mobileMoveVector.y * GAME.playerSpeed * controlFactor;
      hasCommand = true;
      ...
      if (distance > 10) {
        ...
        const commandSpeed = Math.min(GAME.playerSpeed * controlFactor, distance * 3);
    ...
    const accel =
      (hasCommand ? GAME.playerScreenAcceleration : GAME.playerScreenDeceleration) *
      damageFactor;
  ...
  isDirectorRecovery() {
    return (this.combatDirector?.recoveryUntil || 0) > this.time.now;
  }

  addCombatStress(amount) {
    if (!this.combatDirector) return;
    this.combatDirector.stress = Phaser.Math.Clamp(
      this.combatDirector.stress + amount,
      0,
      12
    );
  }
  ...
  updateCombatDirector(dt) {
    if (!this.combatDirector || !this.state.missionStarted) return;
    ...
    const threatTarget =
      visibleEnemies * 0.72 +
      inboundMissiles * 2.45 +
      (this.state.playerDamageState === 'CRITICAL' ? 1.3 : 0);

    const blend = 1 - Math.exp(-1.15 * dt);
    this.combatDirector.stress = Phaser.Math.Linear(
      this.combatDirector.stress,
      threatTarget,
      blend
    );

    const recovering = this.isDirectorRecovery();

    if (
      !recovering &&
      this.combatDirector.stress >= GAME.directorStressThreshold
    ) {
      this.combatDirector.recoveryUntil =
        this.time.now +
        Phaser.Math.FloatBetween(
          GAME.directorRecoveryMinSeconds,
          GAME.directorRecoveryMaxSeconds
        ) * 1000;
      this.combatDirector.wasRecovering = true;
    }

    if (
      this.combatDirector.wasRecovering &&
      !this.isDirectorRecovery()
    ) {
      this.combatDirector.wasRecovering = false;
      this.combatDirector.stress *= 0.55;
    }
  }
  ...
  spawnEnemy() {
    ...
    enemy.avoidanceUntil = 0;
    enemy.avoidanceHeading = null;
    enemy.avoidanceUrgency = 0;
    enemy.avoidanceTargetSpeed = speed;
  ...
  scoreAvoidanceHeading(enemy, heading, target, predictionTime) {
    const playerWorld = this.getPlayerWorldVelocity(false);
    const candidateEnemyVx =
      Math.cos(heading) * enemy.speedMps - playerWorld.x;
    const candidateEnemyVy =
      Math.sin(heading) * enemy.speedMps - playerWorld.y;
    ...
    const rx = target.x - enemy.x;
    const ry = target.y - enemy.y;
    const rvx = targetVx - candidateEnemyVx;
    const rvy = targetVy - candidateEnemyVy;

    return Math.hypot(
      rx + rvx * predictionTime,
      ry + rvy * predictionTime
    );
  }

  getCollisionAvoidance(enemy) {
    if (!enemy?.active || enemy.isDying) return null;

    const pilot = enemy.pilot || PILOT_PROFILES.FIGHTER;
    const now = this.time.now;

    // Mantener la ruptura unas décimas evita "serrucho" izquierda/derecha.
    if (
      enemy.avoidanceUntil > now &&
      Number.isFinite(enemy.avoidanceHeading)
    ) {
      return {
        heading: enemy.avoidanceHeading,
        urgency: enemy.avoidanceUrgency || 0.5,
        targetSpeed: enemy.avoidanceTargetSpeed || enemy.speedMps
      };
    }

    const enemyVx = enemy.body?.velocity.x || 0;
    const enemyVy = enemy.body?.velocity.y || 0;
    const candidates = [{ target: this.player, player: true }];

    // También evita otros Su-57 cercanos. Máximo 10 enemigos: el coste O(n²)
    // es diminuto y elimina cruces visualmente absurdos entre compañeros.
    for (const other of this.enemies.getChildren()) {
      if (
        ...
        candidates.push({ target: other, player: false });
      }
    }
    ...
    for (const item of candidates) {
      ...
      if (
        currentDistance < 1 ||
        currentDistance > GAME.enemyAvoidMaxDetectionDistance
      ) continue;

      const targetVx = target.body?.velocity.x || 0;
      const targetVy = target.body?.velocity.y || 0;
      const rvx = targetVx - enemyVx;
      const rvy = targetVy - enemyVy;
      const relativeSpeedSq = rvx * rvx + rvy * rvy;

      if (relativeSpeedSq < 25) continue;

      const closingProjection = rx * rvx + ry * rvy;
      const tca = -closingProjection / relativeSpeedSq;

      const relativeSpeed = Math.sqrt(relativeSpeedSq);
      const lookAhead =
        Phaser.Math.Linear(
          GAME.enemyAvoidLookAheadMin,
          GAME.enemyAvoidLookAheadMax,
          pilot.skill
        ) +
        Phaser.Math.Clamp(relativeSpeed / 950, 0, 0.45);

      if (tca <= 0.03 || tca > lookAhead) continue;

      const missX = rx + rvx * tca;
      const missY = ry + rvy * tca;
      const missDistance = Math.hypot(missX, missY);

      const baseDistance = item.player
        ? GAME.enemyAvoidPlayerBaseDistance
        : GAME.enemyAvoidTrafficBaseDistance;

      const safeDistance =
        baseDistance +
        pilot.skill * (item.player ? 28 : 16) +
        Math.min(item.player ? 28 : 14, relativeSpeed * 0.045);

      if (missDistance >= safeDistance) continue;

      const urgency =
        (1 - missDistance / safeDistance) *
        (1 - tca / lookAhead);

      if (!bestRisk || urgency > bestRisk.urgency) {
        bestRisk = {
          target,
          player: item.player,
          tca,
    ...
    if (!bestRisk) return null;

    const deflection = Phaser.Math.DegToRad(
      Phaser.Math.Linear(38, 82, Phaser.Math.Clamp(bestRisk.urgency * 1.6, 0, 1))
    );

    const leftHeading = Phaser.Math.Angle.Wrap(enemy.heading - deflection);
    const rightHeading = Phaser.Math.Angle.Wrap(enemy.heading + deflection);
    const predictionTime = Phaser.Math.Clamp(bestRisk.tca, 0.35, 1.35);

    const leftScore = this.scoreAvoidanceHeading(
      enemy,
      leftHeading,
      bestRisk.target,
      predictionTime
    );
    const rightScore = this.scoreAvoidanceHeading(
      enemy,
      rightHeading,
      bestRisk.target,
      predictionTime
    ...
    enemy.avoidanceHeading = chosen;
    enemy.avoidanceUrgency = bestRisk.urgency;
    enemy.avoidanceTargetSpeed = Math.max(
      GAME.enemyMinSpeed,
      Math.min(enemy.speedMps, GAME.enemyTurnSpeed + 20)
    );
    enemy.avoidanceUntil =
      now +
      Phaser.Math.Linear(
        280,
        720,
        Phaser.Math.Clamp(bestRisk.urgency + pilot.skill * 0.25, 0, 1)
      );

    return {
      heading: chosen,
      urgency: bestRisk.urgency,
      targetSpeed: enemy.avoidanceTargetSpeed
    };
  }
  ...
  updateEnemies(dt) {
    for (const enemy of this.enemies.getChildren()) {
      ...
      // Evitación predictiva de colisión tiene prioridad sobre el ataque.
      // No rebota en el jugador: calcula el cruce futuro y comienza una ruptura
      // antes de alcanzar la zona de impacto.
      const avoidance = this.getCollisionAvoidance(enemy);
      if (avoidance) {
        desiredHeading = avoidance.heading;
        gLimit = Math.max(gLimit, enemy.attackG * 1.08);
        enemy.targetSpeedMps = Math.min(
          enemy.targetSpeedMps,
          avoidance.targetSpeed
        );
      } else if (distanceToPlayer < 96 && enemy.phase === 'attack') {
        // Solo como último recurso evitamos pivotar sobre el centro del F-22.
  ...
  updateEnemyMissileSystems(dt) {
    ...
    const now = this.time.now;
    const directorRecovery = this.isDirectorRecovery();
    ...
    for (const enemy of this.enemies.getChildren()) {
      ...
      if (inEnvelope && !directorRecovery) {
        const lockSeconds =
    ...
    for (const missile of this.enemyMissiles.getChildren()) {
      ...
      if (
        ...
        if (flare) {
          ...
          if (!missile.decoyAwarded) {
            missile.decoyAwarded = true;
            this.state.score += 300;
            this.showWeaponCue('MISSILE DECOYED  +300', '#69ff9e');
            this.updateHud();
          }
        }
      }
  ...
  damagePlayer(kind = 'gun') {
    ...
    if (kind === 'missile') {
      damage = Phaser.Math.Between(
        GAME.playerMissileDamageMin,
        GAME.playerMissileDamageMax
      );
    } else if (kind === 'collision') {
      damage = Phaser.Math.Between(
        GAME.playerCollisionDamageMin,
        GAME.playerCollisionDamageMax
      );
    } else {
      damage = Phaser.Math.Between(
        GAME.playerGunDamageMin,
        GAME.playerGunDamageMax
      );
    }

    const previousState = this.state.playerDamageState;
    this.state.playerIntegrity = Math.max(
      0,
      this.state.playerIntegrity - damage
    );

    this.state.playerDamageState =
      this.state.playerIntegrity <= 0 ? 'DESTROYED' :
      this.state.playerIntegrity <= GAME.playerCriticalThreshold ? 'CRITICAL' :
      this.state.playerIntegrity <= GAME.playerDamagedThreshold ? 'DAMAGED' :
      'OK';

    // Conservamos las tres marcas de vida como lectura arcade del estado estructural.
    this.state.lives =
      this.state.playerIntegrity <= 0
        ? 0
        : Math.max(
            1,
            Math.ceil(
              this.state.playerIntegrity /
              (GAME.playerIntegrity / GAME.playerLives)
            )
          );

    this.state.score -= Math.round(
      GAME.hitPenalty * Phaser.Math.Clamp(damage / 50, 0.35, 1.65)
    );
    this.playPlayerHitSound();
    this.addCombatStress(kind === 'missile' ? 3.0 : kind === 'collision' ? 3.8 : 1.7);

    if (this.state.playerIntegrity <= 0) {
      this.updateHud();
      this.endGame();
      return;
    }
    ...
    if (
      previousState !== this.state.playerDamageState &&
      this.state.playerDamageState !== 'OK'
    ) {
      this.showContextTip(
        'damage',
        this.state.playerDamageState === 'CRITICAL'
          ? 'AIRFRAME CRITICAL  //  PERFORMANCE REDUCED'
          : 'AIRFRAME DAMAGED  //  PERFORMANCE SLIGHTLY REDUCED'
      );
    }

    // Mientras quede integridad, un impacto nunca puede esconder o bloquear el F-22.
    this.ensurePlayerOperational();

    if (this.player.body) {
      const maxControlSpeed =
        GAME.playerSpeed *
        (this.state.playerDamageState === 'CRITICAL' ? 0.84 :
         this.state.playerDamageState === 'DAMAGED' ? 0.94 : 1);
    ...
    this.updateHud();
    this.createPlayerHitFx(damage, this.state.playerDamageState);
  ...
  createPlayerHitFx(damage = 0, damageState = 'DAMAGED') {
    ...
    const hitLabel =
      damageState === 'CRITICAL'
        ? `CRITICAL!  -${Math.round(damage)}%`
        : `HIT!  -${Math.round(damage)}%`;

    const hit = this.add.text(this.scale.width / 2, this.scale.height * 0.72, hitLabel, {
  ...
  updateHud() {
    ...
    const integrity = Math.max(0, Math.round(this.state.playerIntegrity));
    const damageTag =
      this.state.playerDamageState === 'CRITICAL' ? 'CRIT' :
      this.state.playerDamageState === 'DAMAGED' ? 'DMG' :
      'OK';

    this.hudLives.setColor(
      this.state.playerDamageState === 'CRITICAL' ? '#ff4b5d' :
      this.state.playerDamageState === 'DAMAGED' ? '#ffcf32' :
      '#70f2ff'
    );

    this.hudLives.setText(
      mode === 'desktop'
        ? `AIR ${integrity}% ${damageTag}  ${'◆'.repeat(Math.max(0, this.state.lives))}${'◇'.repeat(Math.max(0, GAME.playerLives - this.state.lives))}`
        : `${integrity}% ${'◆'.repeat(Math.max(0, this.state.lives))}${'◇'.repeat(Math.max(0, GAME.playerLives - this.state.lives))}`
    );
    ...
    if (this.speedWidget) {
      ...
      this.speedWidget.target.setText(
        kmh !== targetKmh
          ? `▶${targetKmh}`
          : `E${this.state.energyPercent}`
      );

      const speedNorm = Phaser.Math.Clamp(
        (this.state.playerAirspeed - GAME.playerEnergyFloorAirspeed) /
        Math.max(1, GAME.playerMaxAirspeed - GAME.playerEnergyFloorAirspeed),
        0,
        1
      );
    ...
    if (mode === 'micro') {
      ...
      this.hudRight.setText([
        ...
        inboundMissiles
          ? `MISSILE INBOUND  ${inboundMissiles}`
          : `ENERGY ${this.state.energyPercent}%   KILLS ${this.state.destroyed}`
      ]);
    }
