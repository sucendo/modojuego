# System Forge — Changelog

## v1.9.5.0 — Scientific Instruments I + Spacecraft I

- Nueva herramienta Medir A↔B con inspector relativo en vivo.
- Distancia, tiempo-luz, velocidad relativa/radial/tangencial, energía orbital específica, momento angular específico y estado ligado/no ligado.
- Elementos relativos a/e/i, periapsis, apoapsis y periodo cuando son físicamente aplicables.
- Línea 3D A↔B y etiqueta de distancia opcionales; actualización sincronizada con la pose final de cámara.
- Creación y edición de vehículos espaciales desde la UI.
- Nueva capa y categoría de etiquetas Vehículos espaciales.
- Preset Solar ampliado a 75 cuerpos con ISS, Hubble, Parker Solar Probe, Voyager 1 y Voyager 2.
- ISS/Hubble usan estados TLE documentados como condiciones iniciales aproximadas; Parker y Voyager indican explícitamente las partes modeladas/aproximadas.
- Spacecraft I es gravitatorio/balístico: sin maniobras activas, drag, reboost ni consumo dinámico de propelente.
- Ayuda y tour actualizados para Medir A↔B y spacecraft.
- Schema 22 sin cambios.
- Nuevas pruebas de medición, categorías renderer, preset Solar y frame sync.

## v1.9.4.4 — Frame Sync, Adaptive Trails & Spacecraft Foundation

- Sincronización final de overlays: cuerpos → cámara → matrices → iconos/etiquetas/baricentro → render.
- Corregido el desfase visual de iconos y nombres a velocidades de simulación altas.
- Trayectorias N-body con muestreo dependiente del periodo, velocidad relativa y velocidad del simulador.
- El extremo visible de cada trayectoria llega siempre al estado físico actual del astro aunque el histórico tenga una cadencia más espaciada.
- Refresco de trayectorias adaptado a la velocidad angular del cuerpo y a la calidad de render.
- Nueva base `SpacecraftModel` para satélites artificiales, sondas, naves tripuladas, estaciones, landers, rovers, telescopios, carga, repetidores y objetos artificiales.
- El modo N-body reducido incluye `spacecraft` entre los seguidores orbitales jerárquicos.
- JSON/CSV preparados para metadatos de misión y masa/tamaño de vehículos.
- Schema 22; migración desde schema 21 sin remapear `visualScale`.
- Nuevas pruebas de frame sync, trayectoria adaptativa, migración y spacecraft.

## v1.9.4.3 — Runtime, Render & IO Refactor

- Órbitas y trayectorias N-body pasan a módulos de renderer.
- Vista y capas pasa a `LayersController`.
- Import/export y serialización pasan a `src/io/project-io.js`.
- El bucle principal pasa a `src/physics/runtime.js`.
- Schema 21 sin cambios.
- Nuevas pruebas de runtime e IO.
...
- Tags por defecto: CF (Ciencia Ficción), Dune y Voyastris.
...
## v1.9.2.8 — Mobile Orientation, UI Preferences & True Scale

- El modo móvil ya no depende solo de `max-width:720px`: un teléfono sigue usando drawers y controles táctiles al girar de vertical a horizontal.
- La reconciliación responsive conserva selección/cámara y recalcula renderer/canvas sin convertir un móvil apaisado en layout de escritorio.
- Eliminado el botón Centrar duplicado del bloque de zoom móvil.
- ⚙ Opciones permite mostrar/ocultar Zoom móvil, Centrar, Seguir/Libre, Análisis temporal y marcadores de posición del tramo 1:1. Las preferencias son locales de la interfaz y sobreviven al cambio de proyecto.
- Nueva escala 3D: `1:1 físico → Actual → Adaptativa → Local`. De 1:1 a Actual las distancias orbitales permanecen físicas; solo se introduce progresivamente la ayuda visual de tamaño.
- En `1:1 físico` los cuerpos usan su radio real sin mínimo de píxeles. Los marcadores opcionales son ayudas independientes y no alteran la geometría.
- Desde `Actual` hasta `Local` se conserva la curva visual de v1.9.2.7, remapeada al nuevo slider.
- Schema v21 migra automáticamente `visualScale` de proyectos anteriores para preservar su apariencia.

## v1.9.2.9 — Scale & Layer Controls Refinement
- Los puntos de posición 1:1 pasan a ser anillos adaptativos alrededor del cuerpo: transparentes, coloreados por objeto, sensibles al diámetro aparente y ocultos automáticamente cuando el cuerpo ya se ve por sí mismo.
- Escala simplificada a `1:1 físico → Adapt. → Local`; se elimina el hito intermedio `Adaptativa`.
- `Adapt.` sustituye al antiguo `Actual`: a su izquierda las órbitas permanecen 1:1 y solo cambia el tamaño visual; a su derecha se conserva la curva histórica hasta `Local`.
- Auto usa `Adapt.` para vistas globales de estrella/estructura, evitando caer en el hito intermedio eliminado.
- `Órbitas`, `Trayectoria N-body` y `Etiquetas` son botones divididos 75/25 en Vista y capas. La parte principal activa/desactiva; `▾` abre las opciones de esa capa.
- Órbitas: `Órbitas visibles` + `Geometría orbital`.
- Trayectoria: `Trayectoria real N-body` + `Historial de trayectoria`, ahora con 150 / 300 / 600 / 1.200 muestras.
- Etiquetas: checks por categoría dentro de su propio submenú.
- appVersion y presets actualizados a 1.9.2.9; schema permanece en v21 porque el significado numérico de la escala no cambia respecto a v21.

## v1.9.3.0 — Camera Travel & Selectable True-Scale Icons
- Los antiguos anillos 1:1 pasan a ser iconos circulares de posición seleccionables; un clic selecciona el cuerpo y ejecuta el mismo flujo de foco que árbol/mapa 3D.
- Los iconos siguen las categorías activas de Etiquetas y no alteran el tamaño físico. Mantienen opacidad completa hasta 5 px de diámetro aparente del astro y se desvanecen progresivamente hasta desaparecer a 11 px.
- Nueva opción de viaje progresivo al seleccionar con duración configurable entre 0,5 y 8 segundos. Funciona incluso con Cámara libre; al terminar no cambia el modo de cámara.
- El viaje usa easing cúbico y actualiza el destino mientras el cuerpo se mueve durante la simulación. Arrastrar/zoom/centrar cancela el viaje de forma inmediata.
- Órbitas, Trayectoria N-body y Etiquetas usan botones divididos más compactos, con flecha integrada y popover reducido. Etiquetas muestra sus checks en una columna, siguiendo el patrón de la interfaz de referencia.
- La escala 1:1 — Adapt. — Local se conserva exactamente como en v1.9.2.9.

## v1.9.3.1 — Split-control & marker hotfix
- Los controles divididos de Órbitas, Trayectoria N-body y Etiquetas usan ahora el mismo diseño compacto segmentado en escritorio y móvil; el CSS dejó de estar restringido al media query móvil.
- Eliminado el segundo aro decorativo interior de los iconos 1:1. El marcador es ahora un único anillo adaptativo alrededor del cuerpo celeste.

## v1.9.3.2 — Marker Priority & Guide Refresh
- Los anillos 1:1 reutilizan la misma prioridad de solape de las etiquetas: seleccionado primero; después anfitriones/cuerpos con descendencia; después estrellas, planetas, satélites y cuerpos menores. Una Luna solapada por la Tierra se oculta salvo que la Luna esté seleccionada.
- Los iconos siguen siendo clicables y mantienen el fundido por tamaño aparente de 5 a 11 px.
- Ayuda completamente revisada para reflejar escala `1:1 — Adapt. — Local`, botones divididos, viaje de cámara, catálogo local, fast-time y opciones actuales.
- Guía interactiva rehecha con 14 pasos y anclajes vigentes; abre automáticamente paneles/detalles necesarios, funciona mejor en móvil y se reposiciona al redimensionar o girar el dispositivo.
- Etiquetas base renombradas a `CF`, `Dune` y `Voyastris`; se conservan los IDs internos históricos para compatibilidad. Los proyectos que aún usen exactamente `Lore`, `Universo · Dune` o `Universo · Voyastris` migran visualmente a los nuevos nombres.
- appVersion 1.9.3.2; schema permanece en v21.

## v1.9.4.1 — Renderer Refactor

- Segunda fase de modularización sin cambio de schema ni de física.
- Extraídos `orbit-controls`, `camera-navigation`, `overlay-priority`, `label-layer` y `marker-layer`.
- Etiquetas e iconos 1:1 comparten una única prioridad jerárquica, evitando divergencias en los solapes.
- Viaje progresivo, seguimiento y centrado pasan a un controlador de cámara independiente.
- Añadida regresión renderer-smoke para Tierra/Luna, categorías, fade 5→11 px y easing del viaje.
- appVersion 1.9.4.1; schema permanece en v21.

## v1.9.4.0 — Core Refactor & UI Polish
- Primera fase de modularización: EventBus, preferencias, Ayuda/Tour, hover astronómico y controlador del panel de rendimiento.
- Nuevo hover astronómico propio; eliminado el tooltip nativo «Seleccionar …» de los iconos 1:1.
- Ayuda rediseñada con índice independiente, scroll propio, buscador y contenido con scroll separado.
- Tour extraído a módulo y preparado para controles ocultos/responsive.
- Panel de rendimiento ocultable desde Opciones; las métricas internas siguen activas para el fast-time adaptativo.
- Primera suite de regresión permanente y documentación de arquitectura.
- appVersion 1.9.4.0; schema permanece en v21.


## v1.9.4.2 — Physics Refactor

- Tercera fase de modularización sin cambio de schema ni de escala visual.
- Extraído `src/physics/integrator.js`: aceleraciones N-body, Velocity Verlet, energía total y momento angular.
- Extraído `src/physics/structures.js`: gravedad/potencial de estructuras y sincronización con el padre.
- Extraído `src/physics/fast-time.js`: presets temporales, presupuesto adaptativo, full/reduced/Kepler, lunas orbitales y preview Kepler.
- `index.html` conserva adaptadores mínimos para consumidores históricos; la implementación real ya no vive inline.
- Nueva prueba de dos cuerpos durante una órbita: deriva energética y de momento angular bajo tolerancia.
- Nueva prueba de campo de cáscara esférica y regresión de umbrales del selector fast-time.
- appVersion 1.9.4.2; schema permanece en v21.
